...
let state = {
  ...
  authModes: {},          // instId -> 'header' | 'body' = modo de auth por instancia
...
chrome.storage.local.get(['config'], async (r) => {
  if (r.config) {
    ...
    saveConfig(); // persiste migracao se houve
  }
  scheduleAlarm();
...
async function _pollZabbixOnce() {
  ...
  setStatus({
    ...
    problems: allActive
      ...
      .map(p => ({
        ...
        suppressed: p.suppressed === '1', ackmsg: p.ackmsg || '',
  ...
  if (!config.muted) {
    if (allFresh.length) {
      const freshMax = allFresh.reduce((m, p) => Math.max(m, Number(p.severity)), 0);
      if (config.soundEnabled) { playSound(soundForSeverity(freshMax), config.volume); state.lastAlarmTs = now; }
      if (config.notificationsEnabled) {
        allFresh.sort((a, b) => Number(b.severity) - Number(a.severity));
        allFresh.slice(0, MAX_NOTIFS_PER_POLL).forEach(p => {
          const inst = instances.find(i => i.id === p._instId) || instances[0];
          notify(p, normalizeUrl(inst.url));
        });
      }
    }
    // NAG
    if (config.soundEnabled && config.repeatAlarm) {
      const nagPool = allActive.filter(p => p.acknowledged !== '1');
  ...
  if (config.notifyResolved && config.notificationsEnabled && allResolved.length) {
    allResolved.slice(0, MAX_NOTIFS_PER_POLL).forEach(p => {
      const inst = instances.find(i => i.id === p.instId) || instances[0];
      notifyResolved(p, normalizeUrl(inst.url));
    });
  }
}
...
async function _pollInstance(inst) {
  ...
  let active = problems.filter(p => {
    ...
    if (maxAgeMs && (Date.now() - Number(p.clock) * 1000) > maxAgeMs) return false;
    return true;
  });
